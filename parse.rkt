...
(define (make-parser . xcolumn-names)
  (let* ((column-names
         ...
         (templates (list
                      (list '(is-null "(" () ")") (lambda (x) (list 'is-null (list-ref x 2))))
                      (list '(is-not-null "(" () ")") (lambda (x) (list 'is-not-null (list-ref x 2))))
                      (list '(index-of () "in" ()) (lambda (x) (list 'index-of (list-ref x 1) (list-ref x 3))))
                      (list '(substring "(" () "," () "," () ")")  (lambda (x) (list 'substring (list-ref x 2) (list-ref x 4) (list-ref x 6))))
                      ...
                      (list '(concat "(" () "," () ")") (lambda (x) (list 'concat (list-ref x 2) (list-ref x 4))))
                      (list '(between "(" () "," () "," () ")") (lambda (x) (list 'concat (list-ref x 2) (list-ref x 4) (list-ref x 6))))
                      (list '(replace "(" () "," () "," () ")") (lambda (x) (list 'replace (list-ref x 2) (list-ref x 4) (list-ref x 6))))
                      (list '(like "(" () "," () "," () ")")  (lambda (x) (list 'like (list-ref x 2) (list-ref x 4) (list-ref x 6))))
                      (list '(now) (lambda (x) (list x)))
                      (list '(quotient "(" () ")") (lambda (x) (list 'quotient (list-ref x 2))))
                      (list '(quotient ()) (lambda (x) (list x)))
                      (list '(quotient "of" ()) (lambda (x) (list 'quotient (list-ref x 2))))
                      (list '(remainder ()) (lambda (x) (list x)))
                      (list '(remainder "of" ()) (lambda (x) (list 'remainder (list-ref x 2))))
                      (list '(remainder "(" () ")") (lambda (x) (list 'remainder (list-ref x 2))))
                      (list '(abs "of" ()) (lambda (x) (list 'abs (list-ref x 2))))
                      (list '(abs "value" ()) (lambda (x) (list 'abs (list-ref x 2))))
                      (list '(abs "value" "of" ()) (lambda (x) (list 'abs (list-ref x 3))))
                      (list '(abs "(" () ")") (lambda (x) (list 'abs (list-ref x 2))))
                      (list '(round "(" () ")") (lambda (x) (list 'round (list-ref x 2))))
                      (list '(round ()) (lambda (x) (list 'round (list-ref x 1))))
                      (list '(ceiling "of" ()) (lambda (x) (list 'ceiling (list-ref x 2))))
                      (list '(ceiling ()) (lambda (x) (list 'ceiling (list-ref x 1))))
                      (list '(ceiling "(" () ")") (lambda (x) (list 'ceiling (list-ref x 2))))
                      (list '(floor "of" ()) (lambda (x) (list 'floor (list-ref x 2))))
                      (list '(floor ()) (lambda (x) (list 'floor (list-ref x 1))))
                      (list '(floor "(" () ")") (lambda (x) (list 'floor (list-ref x 2))))
                      (list '(truncate ()) (lambda (x) (list 'truncate (list-ref x 1))))
                      (list '(truncate "(" () ")") (lambda (x) (list 'truncate (list-ref x 2))))
                      (list '(sign "(" () ")") (lambda (x) (list 'sign (list-ref x 2))))
                      (list '(sign "of" ())  (lambda (x) (list 'sign (list-ref x 2))))
                      (list '(exponent "(" () "," () ")") (lambda (x) (list 'exponent (list-ref x 2) (list-ref x 4))))
                      (list '(exponent "of" () "raised" "to" ()) (lambda (x) (list 'exponent (list-ref x 2) (list-ref x 5))))
                      (list '(logarithm "(" () ")") (lambda (x) (list 'logarithm (list-ref x 2))))
                      (list '(logarithm "of" ())  (lambda (x) (list 'logarithm (list-ref x 2))))
                      (list '(natural-logarithm "(" () ")") (lambda (x) (list 'natural-logarithm (list-ref x 2))))
                      (list '(natural-logarithm "of" ())  (lambda (x) (list 'natural-logarithm (list-ref x 2))))
                      (list '(natural-logarithm ())  (lambda (x) (list 'natural-logarithm (list-ref x 1))))
                      (list '(sqrt "(" () ")") (lambda (x) (list 'sqrt (list-ref x 2))))
                      (list '(sqrt ()) (lambda (x) (list 'sqrt (list-ref x 1))))
                      (list '(upper "(" () ")") (lambda (x) (list 'upper (list-ref x 2))))
                      (list '(lower "(" () ")") (lambda (x) (list 'upper (list-ref x 2))))
                      (list '(length "(" () ")") (lambda (x) (list 'length (list-ref x 2))))
                      (list '(length ()) (lambda (x) (list 'length (list-ref x 1))))
                      (list '(in-list "(" () "," () ")")  (lambda (x) (list 'in-list (list-ref x 2)(list-ref x 4))))
                      (list '(trim "(" () ")") (lambda (x) (list 'trim (list-ref x 2))))
                      (list '(trim ()) (lambda (x) (list x)))
                      (list '(trim "in" ()) (lambda (x) (list 'trim (list-ref x 2))))
                      (list '(trim "from" ()) (lambda (x) (list 'trim (list-ref x 2))))
                      (list '(average () group ()) (lambda (x) (list 'average-group (list-ref x 1) (list-ref x 3))))
                      (list '(average "(" () ")" group ()) (lambda (x) (list 'average-group (list-ref x 2)(list-ref x 5))))
                      (list '(average "of" () group ()) (lambda (x) (list 'average-group (list-ref x 2)(list-ref x 5))))
                      (list '(average "(" () ")" group ()) (lambda (x) (list 'average-group (list-ref x 2)(list-ref x 5))))
                      (list '(average "of" ()) (lambda (x) (list 'average (list-ref x 2))))
                      (list '(average ()) (lambda (x) (list x)))
                      (list '(minimum () group ()) (lambda (x) (list 'minimum-group (list-ref x 1) (list-ref x 3))))
                      (list '(minimum "(" () ")") (lambda (x) (list 'minimum (list-ref x 2))))
                      (list '(minimum "of" ()) (lambda (x) (list 'minimum (list-ref x 2))))
                      (list '(minimum "in" ()) (lambda (x) (list 'minimum (list-ref x 2))))
                      (list '(minimum ()) (lambda (x) (list 'minimum (list-ref x 1))))
                      (list '(maximum () group ()) (lambda (x) (list 'maximum-group (list-ref x 1) (list-ref x 3))))
                      (list '(maximum "(" () ")") (lambda (x) (list 'maximum (list-ref x 2))))
                      (list '(maximum "of" ()) (lambda (x) (list 'maximum (list-ref x 2))))
                      (list '(maximum "in" ()) (lambda (x) (list 'maximum (list-ref x 2))))
                      (list '(maximum ()) (lambda (x) (list 'maximum (list-ref x 1))))
                      (list '(count () group ()) (lambda (x) (list 'count-group (list-ref x 1) (list-ref x 3))))
                      (list '(count "(" () ")") (lambda (x) (list 'count (list-ref x 2))))
                      (list '(count "in" ()) (lambda (x) (list 'count (list-ref x 2))))
                      (list '(count "of" ()) (lambda (x) (list 'count (list-ref x 2))))
                      (list '(count ()) (lambda (x) (list 'count (list-ref x 1))))
                      (list '(sum () group ()) (lambda (x) (list 'sum-group (list-ref x 1) (list-ref x 3))))
                      (list '(sum "(" () ")") (lambda (x) (list 'sum (list-ref x 2))))
                      (list '(sum "of" ()) (lambda (x) (list 'sum (list-ref x 2))))
                      (list '(sum ()) (lambda (x) (list x)))
                      (list '(add-seconds ()) (lambda (x) (list x)))
                      (list '(add-hours ()) (lambda (x) (list x)))
                      (list '(add-minutes ()) (lambda (x) (list x)))
                      (list '(add-days ()) (lambda (x) (list x)))
                      (list '(add-months ()) (lambda (x) (list x)))
                      (list '(add-years ()) (lambda (x) (list x)))
                      (list '(subtract-seconds ()) (lambda (x) (list x)))
                      (list '(subtract-hours ()) (lambda (x) (list x)))
                      (list '(subtract-minutes ()) (lambda (x) (list x)))
                      (list '(subtract-days ()) (lambda (x) (list x)))
                      (list '(subtract-months ()) (lambda (x) (list x)))
                      (list '(subtract-years ()) (lambda (x) (list x)))
                      (list '(extract "seconds" "from" ()) (lambda (x) 'extract-seconds (list-ref x 3)))
                      (list '(extract "minutes" "from" ()) (lambda (x) 'extract-minutes (list-ref x 3)))
                      (list '(extract "hours" "from" ()) (lambda (x) 'extract-hours (list-ref x 3)))
                      (list '(extract "days" "from" ()) (lambda (x) 'extract-days (list-ref x 3)))
                      (list '(extract "months" "from" ()) (lambda (x) 'extract-months (list-ref x 3)))
                      (list '(extract "years" "from" ()) (lambda (x) 'extract-years (list-ref x 3)))
                      (list '(extract "epoch" "from" ()) (lambda (x) 'date-to-epoch (list-ref x 3)))
                      (list '(extract "date" "from" ()) (lambda (x) 'date-from-epoch (list-ref x 3)))
                      (list '(extract "day" "of" "year" "from" ()) (lambda (x) 'extract-day-of-year (list-ref x 5)))
                      (list '(extract "day" "of" "week" "from" ()) (lambda (x) 'extract-day-of-week (list-ref x 5)))
                      (list '(not ()) (lambda (x) (list x)))
                      )))
