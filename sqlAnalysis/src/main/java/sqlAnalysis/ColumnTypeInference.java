...
public class ColumnTypeInference extends DefaultTraversalVisitor<Integer, Void> {
	...
	protected String visitJoin(Join node, Void context) {
		...
		if (join.isPresent()) {
			if (join.get() instanceof JoinOn) {
				try {
					process(((JoinOn) join.get()).getExpression(), context);
	...
	protected String visitSimpleCaseExpression(SimpleCaseExpression node, Void context) {
		...
		for (int i = whenclauses.size() - 1; i >= 0; i--) {
			process(whenclauses.get(i).getOperand(), context);
			process(whenclauses.get(i).getResult(), context);
		}
		return null;
	}
	...
	protected String visitQuery(Query node, Void context) {
		...
		visitQuerySpecification(querySpec.get(0), context);
		return null;
	}
	...
	public Map<String, Integer> getExpressionsAndTypes() {
		
		propagateTypes();
		
		Map<String, Integer> columns2Types = new HashMap<String, Integer>();
		
		Iterator<TypedNode> nodes = graph.iterator();
		while (nodes.hasNext()) {
			TypedNode n = nodes.next();
			assert n.types.size() == 1;
			columns2Types.put(n.name, n.types.iterator().next());
		}
		return columns2Types;
	}
